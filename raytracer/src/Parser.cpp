...
void Parser::createScene(std::shared_ptr<Scene> scene) 
{
    ...
    if (file.is_open()) {
        ...
        scene->aaSamples = data["AA"]["samples"];

        // Bonus
        scene->useFresnel = data["useFresnel"];
        ...
        // Bonus
        float focalLength = data["camera"]["focalLength"];
        float aperture = data["camera"]["aperture"];
        ...
        // Bonus
        scene->cam->focalLength = focalLength;
        scene->cam->aperture = aperture;
        scene->dofSamples = data["camera"]["dofSamples"];
        ...
        for (json& obj : objs) 
        {
            ...
            else if (objType == "box")
            {
                ...
                std::shared_ptr<AABB> aabb = std::make_shared<AABB>(position, size);
                aabb->name = objName;
        ...
        for (json& light : lights) 
        {
            ...
            if (lightType == "point")
            {
                ...
                populateVectorHelper(attenuation, "attenuation", light);
